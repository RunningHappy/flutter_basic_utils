...
/// 判断字体为中文等，需禁用偏移
///
class AdMusicText extends StatelessWidget {
  final String? text;
  ...
  const AdMusicText.small(String text,
  ...
  const AdMusicText.normal(String text,
      {Key? key, TextStyle? style, bool? enableOffset})
      : this(
            key: key,
            text: text,
  ...
  const AdMusicText.big(String text,
      {Key? key, TextStyle? style, bool? enableOffset})
      : this(
            key: key,
            text: text,
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: DefaultTextStyle(
        style: defaultStyle!,
        child: Text(text!, style: style, maxLines: 5),
      ),
    );
  }
}
