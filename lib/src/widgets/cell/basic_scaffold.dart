...
class BasicScaffoldController extends ValueNotifier<BasicPagePosition> {
  ...
  Future? animateToPage(BasicPagePosition pagePosition) {
    return _onAnimateToPage?.call(pagePosition);
  }

  Future? animateToLeft() {
    return _onAnimateToPage?.call(BasicPagePosition.left);
  }

  Future? animateToRight() {
    return _onAnimateToPage?.call(BasicPagePosition.right);
  }

  Future? animateToMiddle() {
    return _onAnimateToPage?.call(BasicPagePosition.middle);
  }

  Future Function(BasicPagePosition pagePositon)? _onAnimateToPage;
}

class BasicScaffold extends StatefulWidget {
  final BasicScaffoldController? controller;
...
class _BasicScaffoldState extends State<BasicScaffold>
  ...
  Future animateToPage(p) async {
    ...
    switch (p) {
      case BasicPagePosition.left:
        await animateTo(screenWidth!);
        break;
      case BasicPagePosition.middle:
        await animateTo();
        break;
      case BasicPagePosition.right:
        await animateTo(-screenWidth!);
        break;
    }
  ...
  onHorizontalDragEnd(details, screenWidth) {
    ...
    if (vOffset > scrollSpeed && inMiddle == 0) {
      // 去右边页面
      return animateToPage(BasicPagePosition.left);
    } else if (vOffset < -scrollSpeed && inMiddle == 0) {
      // 去左边页面
      return animateToPage(BasicPagePosition.right);
    } else if (inMiddle > 0 && vOffset < -scrollSpeed) {
      return animateToPage(BasicPagePosition.middle);
    } else if (inMiddle < 0 && vOffset > scrollSpeed) {
      return animateToPage(BasicPagePosition.middle);
    }
          ...
          child: const SizedBox(
            ...
            child: Center(
              child: Text(
                ...
                style: TextStyle(
                  color: Colors.white,
                  fontSize: SystemFontSize.normal,
                ),
              ),
            ),
          ),
