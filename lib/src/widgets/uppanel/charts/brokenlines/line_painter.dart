...
class LinePainter extends BasePainter {
  ...
  /// x 轴最小值，最大值，用来计算内部绘制点的 x 轴位置
  double? xDialMin, xDialMax;

  /// y 轴最小值，最大值，用来计算内部绘制点的 y 轴位置
  ...
  /// 辅助线是否为实线，在显示辅助线的时候才有效，false 的话为虚线，默认实线
  bool hintLineSolid;
  ...
  void _initValue() {
    xDialColor ??= const Color(0xFF222222);
    yDialColor ??= const Color(0xFF999999);
    hintLineColor ??= const Color(0xFF222222);
    xDialMin ??= 0;
    xDialMax ??= 1;
  }
  ...
  /// x 轴刻度 & 辅助线
  void _drawXRuler(Canvas canvas, Paint paint, List<PointData> points) {
    if (xDialValues != null && xDialValues!.isNotEmpty) {
      // 获取刻度长度
  ...
  void _drawLine(Canvas canvas) {
    for (var element in _lineCanvasModels) {
      // 阴影区域
      if (element.shadowPaths != null && element.shaderColors != null) {
        for (var shadowPathElement in element.shadowPaths!) {
      ...
      // 路径
      for (var pathElement in element.paths) {
        var pathPaint = Paint()
          ..isAntiAlias = true
      ...
      if (element.points != null) {
        ...
        for (var pointElement in element.points!) {
